// Package route provides a minimal HTTP client for a self-hosted OSRM instance.
...
// NewOSRMClient returns a Client pointing at the given OSRM base URL (e.g. "http://osrm:5000").
// timeout is applied to every HTTP call; zero falls back to 10 s.
func NewOSRMClient(baseURL string, timeout time.Duration) *Client {
	if timeout <= 0 {
...
func convertRoute(mode routing.TransportMode, r *osrmRoute) *RouteResponse {
	...
	return &RouteResponse{
		...
		Mode:     string(mode),
		Primary:  primary,
		Routes:   []RouteOption{primary},
	}
}

func convertSteps(legs []osrmLeg) []RouteInstruction {
	var out []RouteInstruction
	idx := 0
