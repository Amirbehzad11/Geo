...
type RouteRepository interface {
	SaveRouteCalculation(ctx context.Context, req *RouteRequest, resp *RouteResponse) error
}
...
func (s *RouteService) Calculate(ctx context.Context, req *RouteRequest) (*RouteResponse, error) {
	resp, _, err := s.CalculateWithMeta(ctx, req)
	return resp, err
}

func (s *RouteService) CalculateWithMeta(ctx context.Context, req *RouteRequest) (*RouteResponse, RouteMeta, error) {
...
func routeMode(req *RouteRequest) string {
	if req.VehicleType != "" {
...
func buildMultiModalTrainResponse(legs []*routing.MultiModalLeg) *RouteResponse {
...
func buildMultiModalTransitResponse(legs []*routing.MultiModalLeg) *RouteResponse {
...
func buildMultiModalResponse(topMode routing.TransportMode, legs []*routing.MultiModalLeg) *RouteResponse {
	modelLegs := make([]RouteLeg, 0, len(legs))
	...
	for _, leg := range legs {
		...
		modelLegs = append(modelLegs, RouteLeg{
...
// API  It is also called by InternalBackend.ComputeRoute.
func buildRouteResponse(mode routing.TransportMode, routes []*routing.Route) *RouteResponse {
	opts := make([]RouteOption, 0, len(routes))
	...
	resp := &RouteResponse{Mode: string(mode), Routes: opts}
	if len(opts) > 0 {
		resp.Primary = opts[0]
...
func routeInstructions(in []routing.Instruction) []RouteInstruction {
	if len(in) == 0 {
		return nil
	}
	out := make([]RouteInstruction, 0, len(in))
	for _, inst := range in {
		out = append(out, RouteInstruction{
			Index:       inst.Index,
			Type:        inst.Type,
			...
			Location:    RoutePoint{Lat: inst.Location.Lat, Lng: inst.Location.Lng},
...
func (s *RouteService) persistRoute(req *RouteRequest, resp *RouteResponse) {
	if s.repo == nil {
		return
	}
	go func() {
		...
		defer cancel()
		_ = s.repo.SaveRouteCalculation(bgCtx, req, resp)
	}()
}

func (s *RouteService) publishRouteCalculated(tripID int64, resp *RouteResponse) {
