...
func NewRouteHandler(svc *RouteService, authorizers ...TripWriteAuthorizer) *RouteHandler {
	h := &RouteHandler{svc: svc}
...
//	@Success		200		{object}	response.Success{data=RouteResponse}		"Route calculated successfully"
...
func recordRouteErrorMetric(meta RouteMeta, err error) {
...
func logRouteRequest(meta RouteMeta, latency time.Duration, status int) {
