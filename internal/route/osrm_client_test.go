...
func TestRoute_ParsesDistanceAndDuration(t *testing.T) {
	...
	resp, err := NewOSRMClient(srv.URL, 0).Route(context.Background(), routing.ModeCar, 35.6892, 51.389, 35.71, 51.41)
	if err != nil {
...
func TestRoute_UnsupportedMode(t *testing.T) {
	c := NewOSRMClient("http://127.0.0.1:1", 0) // unreachable on purpose
...
func TestRoute_HTTPError(t *testing.T) {
	...
	_, err := NewOSRMClient(srv.URL, 0).Route(context.Background(), routing.ModeCar, 35, 51, 36, 52)
	if err == nil {
