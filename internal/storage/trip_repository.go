...
func (s *ShipmentDB) FindTripsByUserIDs(ctx context.Context, userIDs []int64) (map[int64][]map[string]any, error) {
	...
	query := buildTripsByUserIDsQuery(s, placeholders, startCoords)
...
func buildTripsByUserIDsQuery(s *ShipmentDB, placeholders []string, startCoords string) string {
...
SELECT
    ...
    t."max_height" AS max_height,
    t."max_width" AS max_width,
    t."max_length" AS max_length,
    t."start_address_id" AS start_address_id,
    t."end_address_id" AS end_address_id,
    t."vehicle_type_id" AS vehicle_type_id,
    COALESCE(vt."title", '') AS vehicle_type_title,
    t."user_vehicle_id" AS user_vehicle_id,
